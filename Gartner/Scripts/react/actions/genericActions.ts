...
export const componentData = (componentDescriptor: ComponentDescriptor, operation: string, onComplete: Function = undefined) => {
					...
					if (onComplete) {
						if (newObject.SuccessList.length > 0) {
							const addedObject = newObject
								.ObjectToReturn
								.find(o => o.ID.Value === newObject.SuccessList[0].Id.toString())
